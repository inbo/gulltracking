...
| 21           | pressure | number | air pressure in ??? |
...
| 31           | speed_accuracy | number | accuracy of the speed measurement in ??? |
| 32           | userflag | number | ??? |
| 33           | speed_3d | number | ??? |
| 34           | speed_2d | number | ??? |
| 35           | direction | number | Heading in degrees (0 is north, 90 is east). |
| 36           | altitude_agl | number | ??? |
| 37           | inbo_year | number | Year from the date_time (column 17) field |
| 38           | inbo_month | number | Month from the date_time (column 17) field |
| 39           | inbo_hour | number | Hour from the date_time (column 17) field |
| 40           | inbo_time_diff | number | Time (in seconds) since the birds last GPS fix |
| 41           | inbo_distance_diff | number | Distance (in meters) from the birds last GPS fix |
| 42           | inbo_speed_2d | number | Average two dimensional speed (in meters per seconds) of the bird between the last GPS fix and the current fix |
| 43           | inbo_distance_to_colony | number | Distance (in meters) to the colony (indicated by colony_latitude and colony_longitude) |
| 44           | inbo_sunlight | boolean | `TRUE` if sunlight was available at the given time and location |
| 45           | inbo_outlier | boolean | `TRUE` if according to [this set of rules](8-flag-outliers) the record is considered an outlier and should be removed from the analysis |
| 46           | inbo_corine_value | text | Value of the Corine land use map at the position of the bird |
